...
| **4** | 그레인 배음 | 거리→음색(배음 5개): 가까움=날카로운 배음, 멀음=순수 사인 + 중심와 포커스 |

- 가장 가까운 장애물 거리에 따라 스캔 속도/펄스 속도 동적 조정
- 구멍(바닥 함몰) → 음수 신호로 구분, 돌출 장애물과 다른 음색
- **[4] 전용**: 정면 ±34°(±2열) Gaussian 가중치로 중심와 효과, 2m 이내 장애물 포커스 부스트 최대 2.5배
...
이동     : W/A/S/D + 마우스 시점
오디오   : 1=동시재생  2=vOICe순차  3=거리버즈  4=그레인배음
...
| 구역 | 위치 | 청각 훈련 목표 |
|------|------|---------------|
| **중앙 광장** | 원점 주변 | 개방 공간 + 산발적 장애물 기본 감지 |
| **문 있는 집** | NE (x=10~18, z=-22~-14) | 벽면·문 통과·실내 공간 인지 |
| **긴 골목길** | W (x=-16~-13, z=-22~2) | 좁은 통로 항법, 낮은 턱, 지그재그 회피 |
| **계단+고가** | S (z=7~23) | 높이 변화(2.5m) 오르내리기, 플랫폼 경계 감지 |
| **구멍** | NE 중간 (x=6~12, z=-12~-6) | 함몰 공간(음수 신호) 감지·회피 |
